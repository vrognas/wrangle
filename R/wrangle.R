...
naGroups.data.frame <- function(x, ...) {
  ...
  key <- group_vars(x)
  if (!all(key %in% names(x))) stop("nonexistent groups(s)")
  if (nrow(x) == 0) return(logical(0))
  if (!length(key)) {
    # i.e. no groups, therefore no NA groups
...
#' @family weak
#' @return data.frame
weak.data.frame <- function(x, ...)
  x[naGroups(x, ...) | dupGroups(x, ...), , drop = FALSE]
...
informative.data.frame <- function(x, ...)
